          ...
          products.map(product => (
            <div key={product.id} className="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300" onClick={() => handleProductClick(product.id)}>
              ...
              <div className="flex justify-between mt-4 space-x-1">
                <button className="flex-1 px-2 py-1 bg-[#4338ca] text-white text-xs rounded-md hover:bg-indigo-600 transition-colors duration-200" onClick={(e) => {
                  e.stopPropagation();
                  addToCart(product);
                }}>Add to Cart</button>
                <button className="flex-1 px-2 py-1 bg-blue-500 text-white text-xs rounded-md hover:bg-pink-600 transition-colors duration-200">Wishlist</button>
              </div>
            </div>
          ))
        ) : (
