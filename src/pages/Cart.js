...
const Cart = () => {
  ...
  const updateQuantityInCart = async (productId, newQuantity) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/cart/update', { productId, quantity: newQuantity }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      if (response.status === 200) {
        ...
        const action = newQuantity > items.find(item => item.productDetails.id === productId).quantity ? 'increased' : 'decreased';
        setNotification(`Quantity ${action} successfully`);
            ...
            item.productDetails ? (
              <div key={item.productDetails.id} className="flex items-center justify-between p-4 bg-white shadow-md rounded-md">
                <div className="flex items-center space-x-4">
                  <img src={item.productDetails.image} alt={item.productDetails.title} className="w-32 h-32 object-contain" />
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-2">
                      <button onClick={() => updateQuantityInCart(item.productDetails.id, item.quantity - 1)} className="px-3 py-1 bg-red-500 text-white rounded-lg shadow hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400">-</button>
                      <span className="px-4 py-1 border rounded-lg bg-gray-100 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantityInCart(item.productDetails.id, item.quantity + 1)} className="px-3 py-1 bg-green-500 text-white rounded-lg shadow hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400">+</button>
                    </div>
                  </div>
                </div>
                <button onClick={() => removeItemFromCart(item.productDetails.id)} className="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Remove</button>
              </div>
            ) : (
